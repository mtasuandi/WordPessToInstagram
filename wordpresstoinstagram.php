		...
		if (isset($_POST['submit_wordpresstoinstagram_account'])) {
			if (isset($_POST['wordpresstoinstagram_account_form_nonce']) && wp_verify_nonce($_POST['wordpresstoinstagram_account_form_nonce'], 'wordpresstoinstagram_account_form')) {
				...
				if (empty($idAccount)) {
					$wpdb->query(
                        $wpdb->prepare(
                            "INSERT INTO $tableInstagramAccounts SET username = %s, password = %s, created_at = NOW()",
                            $username,
                            $password
                        )
                    );
					...
					$wpdb->query(
                        $wpdb->prepare(
                            "UPDATE $tableInstagramAccounts SET username = %s, password = %s, updated_at = NOW() WHERE id = %d",
                            $username,
                            $password,
                            $idAccount
                        )
